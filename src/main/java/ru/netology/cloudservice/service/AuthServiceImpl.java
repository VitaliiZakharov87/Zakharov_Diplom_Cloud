...
import ru.netology.cloudservice.exceptions.TokenNotFoundException;
import ru.netology.cloudservice.exceptions.UserNotAuthorizedException;
...
public class AuthServiceImpl implements AuthService {
    ...
    public User.TokenDto login(User.Credentials credentials) {
        ...
        if (user.isEmpty() || !user.get().getCredentials().getPassword().equals(credentials.getPassword()))
            throw new UserNotAuthorizedException(Operations.LOGIN_FAILED, credentials.getLogin());
    ...
    public ResultMessageDto validateToken(Optional<String> token) {
        if (token.isEmpty())
            throw new TokenNotFoundException(Operations.TOKEN_HEADER_ABSENT, "NULL");
        if (getUserByToken(token).isEmpty())
            throw new TokenNotFoundException(Operations.TOKEN_NOT_FOUND_IN_DB, token.get());
